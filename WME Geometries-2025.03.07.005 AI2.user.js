// ==UserScript==
// @name                WME Geometries Mod
// @version             2025.03.07.005
...
<b>KNOWN ISSUES:</b><br>
    ...
    - Draw State Boundary is no longer available (due to SDK changes).<br>
    - Some 3rd Party Data Files may still cause issues for display.<br>
`;
    ...
    function parseFile(layerObj) {
        ...
        function createClearButton(layerObj, layerid, color) { // Pass color here
            ...
            clearButtonObject.style.backgroundColor = color; // Use passed color
        ...
        if (features.length === 0) {
            ...
            $(".clear-layer-button").on("click", function () {
                ...
                releaseColor(this.style.backgroundColor); // Release color on layer removal
                this.remove();
            });
        }
    ...
    function removeGeometryLayers() {
        ...
        for (const l in geometryLayers) {
            console.log("removeGeometryLayers: Removing layer name:", l); // ADDED for debugging
            sdk.Map.removeLayer({ layerName: l });
            sdk.LayerSwitcher.removeLayerCheckbox({ name: l });
        }
        geometryLayers = {};
